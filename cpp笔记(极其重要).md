...
62. 结构体对齐问题，结构体在32位程序下会按照4字节对齐，且是每种类型一次对齐，每种类型一次对齐的意思是，比如连续3个char，并不是12个字节，而是4个字节。假设连续创建两个结构体，在生成的exe或者dll中，两个结构的地址并非一定连续，这个不是因为对齐，而是因为编译器的栈随机化，比如这次的两个结构中间差了4个字节。
