...
```
这个结构应该叫做单IO数据结构，对于这个结构的各个参数，可以一个个来，WSAOVERLAPPED结构是负责接收重叠信息的，需要在投递请求和获取完成端口的时候接收。第二个参数是实际的数据，其中包含了data和len，但是不建议直接使用这个进行数据的接收和返回。第三个Buffer参数则是dataBuf的一个缓存，实际取用数据都应该走这个。第四个参数则是可有可无的一个，对于TCP没有意义，可以删除，对于UDP则需要依靠这个获取客户端地址。其他参数也可以继续自定义，但是有一个准则是，无论如何，这些参数实际起作用的前提是在投递的时候传入。比如，对于地址来讲，WSARecvFrom的时候传入这个结构的地址，才能在接收的时候正常获取地址。